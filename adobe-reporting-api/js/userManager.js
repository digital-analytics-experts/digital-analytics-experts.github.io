var userManagementApi = (function($, MarketingCloud) {
	...
	function importUsers(csvContents) {
		...
		$.each(lines, function(lineIndex, line) {
			...
			deferreds.push(modifyUser(userJson));
		});
		...
		$.each(deferreds, function(i) {
			...
			this.done(function(data) {
				...
				$(document).trigger("add-alerts", {
					message : "Can't update user " + userName + ": " + data.responseJSON.error_description,
					priority : "warning"
				});
			}).always(function() {
	...
	function modifyUser(user) {
		return analyticsClient.makeRequest("Permissions.SaveLogin", user);
	}
		...
		$.each(deferreds, function(i) {
			...
			this.done(function(data) {
				...
				if (++counter == deferreds.length) {
					...
					MarketingCloud.fileSupport.writeFile((createCsvFromArray(userLoginsFull)), "adobe-user-list.csv" , "text/csv");
				}
			});
		});
	}
	...
	function createCsvFromArray(userArray) {
		var csvString = ",login,first_name,last_name,title,email,created,phone_number,is_admin,must_change_password,is_temp,temp_start_date,temp_end_date,group_names,password \n",
			newLine;
	...
	$(function() {
		...
		$(selectors.fileInput.importModifyUsersInput).change(function(e) {
		...
		$(selectors.primary.modifyBtn).on('click', function() {
			analyticsClient = getAnalyticsClient(true);
			importUsers(csvContents);
		});
